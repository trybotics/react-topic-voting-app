...
export default async function compress (file, options) {
  let remainingTrials = options.maxIteration || 100
  ...
  while (compressedFile.size > maxSizeByte) {
    const newWidth = canvas.width * 0.99
    const newHeight = canvas.height * 0.99
...
export async function canvasToFile (canvas, fileType, fileName, fileLastModified, quality = 1) {
  let file
  if (window.OffscreenCanvas && typeof window.OffscreenCanvas === 'function' && canvas instanceof window.OffscreenCanvas) {
